...
:END:
while sleep 2;  do /usr/bin/emacs --batch -l org -eval '(org-babel-tangle-file "atac_seq.org")' && snakemake --configfile config/int_${HOSTNAME}.yaml --cores 14 --snakefile workflow/int_test.smk --rerun-incomplete; done
...
main "$@"
/usr/bin/emacs --batch -l org -eval '(org-babel-tangle-file "/tmp/test.org")'
#+end_src
***** Methods writeup
:PROPERTIES:
:ID:       85e23df0-1352-4e67-8df5-a10da50c6751
:CUSTOM_ID: 85e23df0-1352-4e67-8df5-a10da50c6751
:END:

/Nextera adapters were trimmed from ATAC-seq libraries using fastp cite:chen2018b and trimmed reads were aligned using bowtie2 cite:langmead2012. Prior to peak calling, PCR duplicates were removed using samtools cite:li2011samtools and reads not mapping to autosomes were removed. Finally reads were downsampled to 9 million read pairs. Peaks were called with MACS2 cite:zhang2021macs2 using either broad peaks for total open chromatin measurements or narrowpeaks for differential chromatin accessibility. Peaks were filterd as described in cite:corces2018. Libraries which did not contribute significantly to the final peak set were removed from analysis (Fig. ref:fig_human_corces_filt, ref:fig_mouse_corces_filt). Filtered peaks were  counted using BAMscale. Variation was removed using RUVseq with the number of modeled factors chosen between 1 and 4 for each differential expression model based on Wilcoxon rank sum test and Spearman correlation of log2-fold change of the original and RUVseq-adjusted counts (Fig. ref:fig_human_ruv_cor_ir1w_sham1w, ref:fig_human_ruv_cor_ir2w_sham2w, ref:fig_mouse_ruv_cor_ir2d_sham, ref:fig_mouse_ruv_cor_ir6w_sham, ref:fig_mouse_ruv_cor_ir6w_ir2d). While log2-fold changes were preserved, cohort grouping by PCA improved substaintally (Fig. ref:fig_human_ruv_pca, ref:fig_mouse_ruv_pca)./
...
Alignment is per cite:corces2018, fragment lengths <= 2000, very sensitive
...
rule bamscale_corces_mouse:
    ...
    shell:
        ...
        """
#+end_src
#+begin_src bash :tangle ./scripts/mouse_bamscale.sh
#!/usr/bin/env bash
...
rule bamscale_corces_human:
    ...
    shell:
        ...
        """
#+end_src
#+begin_src bash :tangle ./scripts/human_bamscale.sh
#!/usr/bin/env bash
...
rule peak_annotation:
    ...
    shell:
        ...
        """
#+end_src
- [[file:./scripts/peak_annotation.R][Rscript]]
...
***** INPROCESS Motif enrichment with Homer
...
- atac peak characterization per cite:duijvenboden2019 see method and fig 5F
...
** ATAC-seq environmental YAML
#+begin_src bash
conda env create -f config/atac_env.yaml
#+end_src
...
** Snakemake configuration YAMLs
:PROPERTIES:

:END:
*** Common
#+begin_src bash :tangle ./config/int_common.yaml
atac_peak_cut: 2
#+end_src
*** Per HOSTNAME
**** [[file:config/int_radonc-cancerbio.yaml][radonc-cancerbio]]
:PARAMETERS:
:header-args:bash: :tangle ./config/int_radonc-cancerbio.yaml
:END:
#+begin_src bash :tangle ./config/int_radonc-cancerbio.yaml


atac_container: "/home/jeszyman/sing_containers/atac.1.1.0.sif"
atac_repo: "/home/jeszyman/repos/atac-seq"
autosome_bed: "/home/jeszyman/repos/atac-seq/test/inputs/mm10chrs.bed"
blacklist_bed: "/home/jeszyman/repos/atac-seq/test/inputs/mm10-blacklist.v2.bed"

#conda_environment: "/home/jeszyman/repos/atac-seq/config/atac_env.yaml"
...
atac_keep_bed: "resources/keep.bed"
...
## ATAC-seq
dir:
  atac_fastq_raw: "test/fastq-atac-raw"
  atac_fastq_proc: "test/fastq-atac-proc"
#+end_src
** [[file:workflow/atac_read_processing.smk][ATAC-seq read and alignment processing]]                               :smk:
:PROPERTIES:
:header-args:snakemake: :tangle ./workflow/atac_read_processing.smk
:END:
*** Description
Sequencing read adapters were removed and reads were quality trimmed using fastp supercite:chen2018b. Read quality was manually assessed using fastqc supercite:andrews2010.

Processed reads were aligned to mm10 using bowtie2 supercite:langmead2012.

PCR duplicate reads were removed and reads were filtered using samtools supercite:li2011samtools. Reads were filtered and processed for ATAC-seq analysis as follows: only paired reads aligning to genome reference autosomes were retained. Finally, alignments were shifted on the forward strand by +4 bp and on the reverse strand by −5 bp to account for the 9-bp duplication introduced by Tn5.
...
*** INPROCESS Make bowtie2 index                                   :smk_rule:
- Snakemake
  #+begin_src snakemake
# Make bowtie2 index
rule atac_index:
...
:END:
**** De-duplicate alignments                                       :smk_rule:
- Snakemake
  #+begin_src snakemake
...
rule dedup:
    ...
    params:
        script  = atac_script_dir + "/dedup.sh",
	threads = threads,
...
rule filter_bam:
    ...
    params:
        script = atac_script_dir + "/filter_bam.sh",
	threads = threads,
    shell:
        """
        {params.script} \
...
- [[file:./workflow/scripts/filter_bam.sh][Shell script]]
  #+begin_src bash :tangle ./scripts/filter_bam.sh
#!/usr/bin/env bash
...
#+end_src
** INPROCESS ATAC-seq quality control
:PROPERTIES:
:header-args:snakemake: :tangle ./workflow/atac_qc.smk
:END:
*** TODO FastQC :smk_rule:                                         :smk_rule:
- Snakemake
  #+begin_src snakemake
rule fastqc:
...
rule alignment_qc:
    ...
    params:
        script = atac_script_dir + "/alignment_qc.sh",
        threads = threads,
    shell:
        """
        {params.script} \
...
rule atacseq_qc:
    ...
    params:
        script = atac_script_dir + "/atacseq_qc.R",
    shell:
        """
        Rscript {params.script} \
        "{input.dup_bams}" \
...
#+end_src
- [[file:workflow/scripts/atac-seq_qc.R][Rscript]]
  #+begin_src R :tangle ./scripts/atacseq_qc.R
#!/usr/bin/env Rscript
args = commandArgs(trailingOnly = TRUE)
dup_bam_str = args[1]
proc_bam_str = args[2]
...
rule atac_multiqc:
    ...
    params:
        out_dir = qc_dir,
        script = atac_script_dir_dir + "/multiqc.sh"
    shell:
        """
        {params.script} \
        {input} {params.out_dir} &> {log}
...
:PROPERTIES:
:header-args:snakemake: :tangle ./workflow/int_test.smk :tangle-mode (identity #o444))
:END:
*** Preamble
#+begin_src snakemake
...
** Differential chromatin accessibility
:PROPERTIES:
:header-args:snakemake: :tangle ./workflow/atac_dca.smk :tangle-mode (identity #o555)
:END:
*** Make R txdb database                                           :smk_rule:
- Snakemake
  #+begin_src snakemake
rule make_txdb:
    container: atac_container,
    ...
    shell:
        ...
        """
#+end_src
- [[file:./scripts/make_txdb.R][Rscript]]
  #+begin_src R :noweb yes :tangle ./scripts/make_txdb.R
#!/usr/bin/env Rscript
args = commandArgs(trailingOnly = TRUE)
gtf_file = args[1]
txdb_file = args[2]
...
#+end_src
** [[file:workflow/atac_peaks.smk][ATAC-seq peak analysis]]                                               :smk:
:PROPERTIES:
:header-args:snakemake: :tangle ./workflow/atac_peaks.smk :tangle-mode (identity #o555)
:END:
*** MACS2 peak processing
**** MACS2 narrow                                                  :smk_rule:
- Snakemake
  #+begin_src snakemake
rule macs2_narrow:
    container: atac_container,
...
macs2 callpeak --treatment $inbam \
      ...
      --nomodel
#+end_src
- Function- Narrow peak as in cite:corces2018 and cite:hendrickson2017
**** MACS2 broad                                                   :smk_rule:
- Snakemake
  #+begin_src snakemake
rule macs2_broad:
    container: atac_container,
...
rule peak_annotation:
    ...
    shell:
        ...
        """
#+end_src
- [[file:./scripts/peak_annotation.R][Base script]]
  #+begin_src R :noweb yes :tangle ./scripts/peak_annotation.R
...
rule make_atac_keep_bed:
    ...
    shell:
        ...
        """
#+end_src
*** do this filt at bedfile step Read pairs were also removed if they overlapped known problematic regions from the ENCODE blacklist_bed [[#amemiya2019][amemiya2019]].
** README
