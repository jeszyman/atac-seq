...
#+end_src
**** Writeup
ATAC-seq sequencing libraries were processed using the pipelines in  https://github.com/jeszyman/atac-seq.

ATAC-seq reads were adapter and quality-trimmed using fastp. Reads were aligned with bowtie2 with parameters as in cite:corces2018 (maxins 2000 very-sensitive). PCR duplicates were removed from aligned reads using samtools. Peak calling on deduplicated reads was performed with MACS2.

Libraries containing more than 9 million reads were retained for further analysis.
...
peak_list = ['/tmp/test1.narrowPeak', '/tmp/test2.narrowPeak']
dfs = [pd.read_csv(file, sep='\t', header=None) for file in peak_list]

merged = merge_peaks(dfs)
reduced = iterative_score_filter(merged)
...
def iterative_score_filter(merged_df):
    merged_df.columns = ['chrom', 'start', 'end', 'name', 'score', 'strand', 'signalValue', 'pValue', 'qValue', 'peak', 'spm']
    sorted_df = merged_df.sort_values('spm', ascending=False)

    interval_trees = {}
    for _, row in sorted_df.iterrows():
        chrom, start, end, spm = row['chrom'], row['start'], row['end'], row['spm']
        if chrom not in interval_trees:
            interval_trees[chrom] = IntervalTree()
        overlaps = interval_trees[chrom][start:end]
        if not overlaps:
            interval_trees[chrom].addi(start, end, row)
        else:
            # Keep only the peak with the highest spm in case of overlaps
            max_overlap_spm = max([interval.data['spm'] for interval in overlaps])
            if spm > max_overlap_spm:
                # Remove existing overlapping intervals
                for interval in overlaps:
                    interval_trees[chrom].discard(interval)
                # Add new interval
                interval_trees[chrom].addi(start, end, row)

    # Extract non-overlapping peaks from interval trees
    non_overlapping_peaks = []
    for tree in interval_trees.values():
        for interval in tree:
            non_overlapping_peaks.append(interval.data)

    # Creating DataFrame from non-overlapping peaks
    non_overlapping_df = pd.DataFrame(non_overlapping_peaks)

    return non_overlapping_df

# Use the function with your DataFrame
filtered_peaks_df = iterative_score_filter(merged)

def id_reproductible_peaks(sig_filt_peaks_df, ):

    """
        # Identify reproducible peaks <DOC STRING>
...
To identify reproducible peaks from this merged peak set, the individual “sample peak sets” were
overlapped with the merged peak set. Peaks from the merged peak set that were observed in at
least two samples with a score per million value >=5 were labeled as reproducible. Lastly, we
removed any peaks that spanned a genomic region containing “N” nucleotides and any peaks
mapping to the Y chromosome. This resulted in a set of high quality, reproducible, fixed-width
peaks for each cancer type (105,585 mean, 32,888 sd) that we refer to as a “cancer type-specific
peak set”.
