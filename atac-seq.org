...
:header-args:snakemake: :tangle no
:END:
**** paralleize copy in rule bamscale:
**** Downsample
#+begin_src snakemake
rule downsample_bam:
    input: f"{atac_dir}/{{species}}/bams/{{library}}_{{build}}_filt.bam",
...
rule atac_edger_dca:
    ...
    output: f"{atac_dir}/{{contrast}}/{{contrast}}.tsv",
    params:
