...
- see
  ...
  - [[id:cf13d347-1d0e-4858-a016-0a8a8af22609][Biotools conda yaml]]
...
# Notes:
#
# This file contains ONLY ATAC-seq-specific packages. For full functionality.
# For full functionality, you must also update this environment with basecamp
# and biotools environmental YAMLs which can be found at
...
dependencies:
  - bamscale                   # Quantifies and normalizes coverage from peaks #
  - bioconductor-atacseqqc                      # ATAC-seq-specific QC program #
  - bioconductor-GenomicRanges     # Data structure for genomic intervals in R #
  - bioconductor-TxDb.Hsapiens.UCSC.hg38.knownGene      # Annotations for hg38 #
  - bioconductor-TxDb.Mmusculus.UCSC.mm10.knownGene     # Annotations for mm10 #
  # - bioconductor-chippeakanno # Peak file annotation #
  # - bioconductor-chipseeker # Additional peak file annotation #
  # - bioconductor-clusterprofiler # Enrichment tool #
  # - bioconductor-csaw # Differential accessibility or binding #
  # - bioconductor-fgsea # Gene set enrichment analysis #
  # - bioconductor-ggbio # Sequence track visualizations w/ ggplot #
  # - bioconductor-gviz # More sequence track visualization #
  # - bioconductor-orthology.eg.db # Mapping genes across species from NCBI gene IDs #
  # - bioconductor-rgreat # Peak set enrichment #
  # - bioconductor-ruvseq
  # - bioconductor-soggi
  - homer
  - macs2
...
conda env update --name your_env_name --file updated_env.yaml
...
- Incorporate as submodule or symlink to basecamp and biotools repositories

Necessary initial directory structure within your declared "atac_repo":

#+BEGIN_EXAMPLE
<REPO_DIR>/
|-- basecamp (from https://github.com/jeszyman/basecamp)
|-- biotools (from https://github.com/jeszyman/biotools)
#+END_EXAMPLE
...
*** Writeup
:PROPERTIES:
:ID:       419aae08-2db5-4ab0-8951-7dcff4d48fcc
:CUSTOM_ID: 419aae08-2db5-4ab0-8951-7dcff4d48fcc
:END:
ATAC-seq reads were processed using pipelines in the repository https://github.com/jeszyman/atac-seq.
...
**** Writeup
:PROPERTIES:
:ID:       b0816013-f51e-404d-971a-44311340181a
:CUSTOM_ID:       b0816013-f51e-404d-971a-44311340181a
:END:

ATAC-seq read alignment, peak calling, and peak filtering followed the method in Corces /et al/., 2018 cite:corces2018. ATAC-seq reads were adapter and quality-trimmed using fastp [[cite:chen2018fastp]]. Reads were aligned with bowtie2 [[cite:&langmead2012]] with parameters as (maxins 2000 very-sensitive). PCR duplicates were removed from aligned reads using samtools [[cite:&li2011samtools]]. Peak calling on deduplicated reads was performed with MACS2 [[cite:&zhang2021macs2]] (--call-summits --extsize 150 --keep-dup all --nolamba -p 0.01 --shift 75 --SPMR --nomodel). For comparisons between libraries, each library's peaks were standardized as follows: peak length was redefined as 250 base pairs upstream and downsteam of the original peak summit. Re-defined peaks were then removed if they extended beyond chromosome length, or if they had any overlap with blacklisted regions as defined by Amemiya /et al/., 2019 [[cite:&amemiya2019]]. Peaks of each library overlapping each other were removed iteratively by selecting the peak with the most significant p-value. Finally, the remaining, non-overlapping standardized peaks were assigned a normalized significance score, score per million, defined as the -log10 p-value of each peak divided by the sum of all -log10 p-values in the library divided by 1 million.
...
#+begin_src python :tangle ./scripts/corces_peak_filter.py
#!/usr/bin/env python3
...
option_list <- list(
  make_option(c("--blacklist_bed"),
              type = "character",
              default = "~/cards/ref/mm10_blklist.bed.gz"),
  make_option(c("--chrom_bed"),
              type = "character",
              default = "~/cards/ref/mm10_chrom.bed"),
  make_option(c("--macs2_narrowPeak"),
              type = "character",
              default = "~/cards/analysis/atac/peaks/lib051_mm10_filt_peaks.narrowPeak")
)
...
blklist = read_tsv(blacklist_bed,
                   col_names = c("chrom", "start", "end"))

chrs = read_tsv(chrom_bed,
                col_names = c("chr", "start", "end"))

in_peaks <- read_tsv(macs2_narrowPeak,
                    col_names = c("chrom",
                                  "chromStart",
                                  "chromEnd",
                                  "name",
...
adj_peaks <- in_peaks %>%
  mutate(new_start = chromStart + peak - 250,
         new_end = chromStart + peak + 250) %>%
  left_join(chrs, by = c("chrom" = "chr")) %>%
  filter(new_end < end) %>%
  filter(new_start > 0) %>%
  select(-start, -end)

gr_blklist <- GRanges(seqnames = blklist$chrom,
                        ranges = IRanges(start = blklist$start, end = blklist$end))

gr_adj <- GRanges(seqnames = adj_peaks$chrom,
                  ranges = IRanges(start = adj_peaks$chromStart, end = adj_peaks$chromEnd))

# Find overlaps
overlaps <- findOverlaps(gr_adj, gr_blklist)

# Filter out peaks that overlap with the blacklist
filt_peaks <- adj_peaks[-queryHits(overlaps), ] %>% arrange(pValue)


gr_filt <- GRanges(seqnames = filt_peaks$chrom,
                  ranges = IRanges(start = filt_peaks$chromStart, end = filt_peaks$chromEnd))


overlaps = findOverlaps(gr_filt, gr_filt, type = "within")

overlaps_filtered <- overlaps[queryHits(overlaps) != subjectHits(overlaps)]

overlaps_filtered

test = gr_filt[queryHits(overlaps_filtered)]
...
gr_peaks <- GRanges(seqnames = final_peaks$chrom,
                           ranges = IRanges(start = final_peaks$new_start, end = final_peaks$new_end))

# Convert the peaks to GRanges
overlap_hits <- findOverlaps(gr_peaks)

# Filter out self-overlaps
real_overlaps <- overlap_hits[queryHits(overlap_hits) != subjectHits(overlap_hits)]

# Get unique indices of overlapping peaks
overlapping_peaks <- unique(c(queryHits(real_overlaps), subjectHits(real_overlaps)))
...
overlap_hits <- findOverlaps(gr_peaks, ignoreSelf = TRUE, ignoreRedundant = TRUE)
overlapping_peaks <- unique(c(queryHits(overlap_hits), subjectHits(overlap_hits)))

overlap_hits <- findOverlaps(gr_peaks)
overlapping_peaks <- unique(c(queryHits(overlap_hits), subjectHits(overlap_hits)))

# Filter to only overlapping peaks and sort by significance
overlapping_sorted <- final_peaks[overlapping_peaks, ] %>%
  arrange(pValue)

# Convert to GRanges for processing
gr_overlapping_sorted <- GRanges(seqnames = overlapping_sorted$chrom,
                                 ranges = IRanges(start = overlapping_sorted$new_start, end = overlapping_sorted$new_end))

# Initialize an empty GRanges object for selected peaks
selected_peaks <- GRanges()

# Process only the overlapping peaks
for (i in seq_along(gr_overlapping_sorted)) {
  current_peak <- gr_overlapping_sorted[i]

  if (length(findOverlaps(current_peak, selected_peaks)) == 0) {
    selected_peaks <- c(selected_peaks, current_peak)
  }
}

80
# Extract indices of selected peaks
selected_indices <- match(selected_peaks, gr_overlapping_sorted)

# Get the final set of non-overlapping peaks
final_overlapping_peaks <- overlapping_sorted[selected_indices, ]

# Combine with non-overlapping peaks
non_overlapping_peaks <- adj_peaks[-overlapping_peaks, ]
final_peaks <- rbind(non_overlapping_peaks, final_overlapping_peaks) %>%
  arrange(chrom, new_start)  # or arrange by your preferred order

# Extract the indices of the selected peaks
selected_indices <- match(selected_peaks, gr_sorted_peaks)

# Get the final set of non-overlapping peaks from the original dataframe
final_peaks2 <- final_peaks[selected_indices, ]
final_peaks2
...
# Assuming adj_peaks is your dataframe of adjusted peaks

# Split data by chromosome
peaks_by_chrom <- split(adj_peaks, adj_peaks$chrom)

# Define the function to process each chromosome's data
process_chromosome <- function(peaks) {
  # Convert to GRanges object
  gr_peaks <- GRanges(seqnames = peaks$chrom,
                      ranges = IRanges(start = peaks$new_start, end = peaks$new_end))
  ...
  # Iteratively select peaks
  for (i in seq_along(sorted_peaks)) {
    current_peak <- gr_peaks[i]
  ...
  # Extract indices of selected peaks
  selected_indices <- match(selected_peaks, gr_peaks)

  # Return the final set of non-overlapping peaks
  sorted_peaks[selected_indices, ]
}
...
clusterExport(cl, list("process_chromosome", "adj_peaks"))

# Apply the function in parallel
final_peaks_list <- parLapply(cl, peaks_by_chrom, process_chromosome)

# Stop the cluster
stopCluster(cl)

# Combine the results
final_peaks <- bind_rows(final_peaks_list)
#+end_src
...
**** Aggregate read and alignment level QC
#+begin_src R
#!/usr/bin/env Rscript
    ...
    run:
        ...
        with open(tsv_file, "w", newline="") as f:
            ...
            for library, q30_rate in zip(libraries, q30_rates):
                writer.writerow([library, q30_rate])
#+end_src
**** Make a list of libraries that meet a QC standard
#+begin_src snakemake
rule atac_keep_libs:
    input:
...
#+begin_src R :tangle ./scripts/atac_keep_libs.R
#!/usr/bin/env Rscript
...
**** Writeup
:PROPERTIES:
:ID:       16f0fd39-8963-41a6-a903-72de4f9f44f0
:CUSTOM_ID:       16f0fd39-8963-41a6-a903-72de4f9f44f0
:END:
To compare differential chromatin accessibility across groups, we again followed the method outlined in Corces /et al/., 2018 cite:corces2018. Standardized peaks from all libraries in the comparison were merged.  As for individual libaries, the merged peak set was then iteratively filterd to removed overlapping peaks, here selecting the peak with the highest score per million. Non-overlapping peaks were then retained only with their score per million was >= 5 in >= 2 samples. Counts of each of these final, high-confidence peaks were then made for the orignal alignments using BAMscale [[cite:&pongor2020bamscale]]. Count-level quality control included principal component analysis. Differential chromatin accessibility was determined in EdgeR using glmQLFTest for individual cohort contrasts. Peak sets were annotated with ChIPseeker [[cite:&yu2015chip]].
...
write_tsv(out,
          ...
          col_names = F)
#+end_src
**** TODO Open genome by chromosome and chromatin state
...
#f"{atac_dir}/peaks/{{library}}_{{build}}_{{bam_set}}_peaks.{{peaktype}}Peak_anno.bed",
anno_peak_file_list = c("~/cards/analysis/atac/peaks/lib051_mm10_filt_peaks.narrowPeak_anno.bed",
                        "~/cards/analysis/atac/peaks/lib051_mm10_filt_peaks.narrowPeak_anno.bed")

#########1#########2#########3#########4#########5#########6#########7#########8

column_names = c("chrom", "start", "end", "name", "score", "strand",
                 "signalValue", "pValue", "qValue", "peak", "genomic_feature",
                 "gene_chr", "gene_start", "gene_end", "gene_length", "gene_strand",
                 "entrez_gene_id", "distance_tss")

anno_peak_df_list = map(anno_peak_file_list, ~read_tsv(.x, col_names = column_names))

q_filt = 10


test = anno_peak_df_list[[1]] %>% filter(qValue > q_filt)

anno_peak_df_list = lapply(anno_peak_file_list, read_tsv)
...
#!/usr/bin/env python3
"""
Script to quantify total open genome in an ATAC-seq library
"""
...
def load_inputs():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--peak_file_list", nargs="+")

    args = parser.parse_args()


    column_names = ['chrom', 'start', 'end', 'name', 'score', 'strand', 'signalValue', 'pValue', 'qValue', 'peak', 'genomic_feature','gene_chr', 'gene_start', 'gene_end', 'gene_length', 'gene_strand', 'entrez_gene_id', 'distance_tss']

    peak_df_list = [pd.read_csv(file, sep="\t", header=None, names=column_names) for file in args.peak_file_list]
...
args = Args()
args.peak_file_list = ['~/cards/analysis/atac/peaks/lib051_mm10_filt_peaks.narrowPeak_anno.bed', '~/cards/analysis/atac/peaks/lib052_mm10_filt_peaks.narrowPeak_anno.bed']

# Filter each DataFrame in peak_df_list for qValue > 5
filtered_peak_df_list = [df[df['qValue'] > 5] for df in peak_df_list]
...
#+begin_src python :tangle ./scripts/corces_group_peak_filter.py
#!/usr/bin/env python3
"""
...
rule atac_combat_batch_correction:
    ...
    shell:
        ...
        """
#+end_src
#+begin_src R :tangle ./scripts/atac_combat_batch_correction.R
#!/usr/bin/env Rscript
...
**** Make RUV-adjusted ATAC-seq counts :smk_rule:
...
#+end_src
*** TODO Fraction of reads in peaks (FRiP)
:PROPERTIES:
